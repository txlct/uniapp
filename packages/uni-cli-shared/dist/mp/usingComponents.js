...
function getComponentPlaceholder(bindingComponents, normalizeComponentName) {
    return Object.values(bindingComponents).reduce((acc, { tag = '', value = '' }) => ({
        ...acc,
        [normalizeComponentName((0, shared_1.hyphenate)(tag))]: value
    }), {});
}
...
function updateMiniProgramComponentsByMainFilename(mainFilename, inputDir, normalizeComponentName) {
    ...
    const filename = (0, utils_1.removeExt)((0, utils_1.normalizeMiniProgramFilename)(mainFilename, inputDir));
    (0, jsonFile_1.addMiniProgramUsingComponents)(filename, createUsingComponents(bindingComponents, imports, inputDir, normalizeComponentName));
    bindingComponentPlaceholder && (0, jsonFile_1.addMiniProgramComponentPlaceholder)(filename, getComponentPlaceholder(bindingComponentPlaceholder, normalizeComponentName));
}
 ...
 */
function parseComponents(ast, propKeyName = 'components') {
    const bindingComponents = {};
    ...
    estree_walker_1.walk(ast, {
        enter(child) {
            ...
            componentsExpr.properties.forEach((prop) => {
                ...
                const name = (0, types_1.isStringLiteral)(prop.value)
                    ? tag || ''
                    : prop.value.name;
